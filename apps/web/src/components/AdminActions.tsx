...
async function mutate(path: string, body: unknown, role = "super_admin") {
  const response = await fetch(`${apiBase}${path}`, { 
    method: path.includes("/assign") || path.includes("/quote") ? "POST" : "PATCH", 
...
export function AssignmentForm({ pickups, collectors }: { pickups: Pickup[]; collectors: Collector[] }) {
  ...
  return <form className="form" onSubmit={async (event) => { 
    ...
    try {
      await mutate(`/pickups/${form.get("pickupId")}/assign`, { collectorId: form.get("collectorId") }, "operations_admin"); 
      toast.success(`Pickup ${form.get("pickupId")} assigned successfully!`);
      setMessage("Pickup assigned. Refresh to see updated board."); 
    } catch (err: any) {
      toast.error(`Assignment failed: ${err.message || err}`);
      setMessage("Failed to assign pickup.");
    }
  }}>
...
export function CollectorApprovalForm({ collectors }: { collectors: Collector[] }) {
  ...
  return <form className="controls" onSubmit={async (event) => { 
    event.preventDefault(); 
    const form = new FormData(event.currentTarget); 
    try {
      const status = form.get("status");
      await mutate(`/collectors/${form.get("collectorId")}/approval`, { status }, "operations_admin"); 
      toast.success(`Collector status updated to ${status}!`);
      setMessage("Collector status updated. Refresh for the latest list."); 
    } catch (err: any) {
      toast.error(`Approval update failed: ${err.message || err}`);
      setMessage("Failed to update collector status.");
    }
  }}>
...
export function SackPricingForm({ sacks }: { sacks: SackType[] }) {
  ...
  return <form className="form" onSubmit={async (event) => { 
    ...
    const form = new FormData(event.currentTarget); 
    try {
      await mutate(`/sack-types/${form.get("sackId")}`, { price: Number(form.get("price")), stockReceived: Number(form.get("stockReceived")) }, "operations_admin"); 
      toast.success("Sack inventory and pricing updated successfully!");
      setMessage("Sack inventory updated. Refresh to verify totals."); 
    } catch (err: any) {
      toast.error(`Sack update failed: ${err.message || err}`);
      setMessage("Failed to update sack inventory.");
    }
  }}>
...
export function PlatformSettingsForm({ settings }: { settings: PlatformSettings }) {
  ...
  return <form className="form" onSubmit={async (event) => { 
    ...
    const form = new FormData(event.currentTarget); 
    try {
      await mutate("/platform/settings", { assignmentMode: form.get("assignmentMode"), collectorLocationIntervalSeconds: Number(form.get("collectorLocationIntervalSeconds")), smsProvider: form.get("smsProvider"), proofPhotosRequired: Number(form.get("proofPhotosRequired")) }, "super_admin"); 
      toast.success("Platform settings saved successfully!");
      setMessage("Platform settings saved. Refresh to reload configuration."); 
    } catch (err: any) {
      toast.error(`Settings update failed: ${err.message || err}`);
